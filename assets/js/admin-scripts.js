jQuery(document).ready(function($){
	...
	function migprog_select($type, $label, $id, $values, $default){
		var $opt = '<div class="migajaxmanager_option_wrapper migprog_hidden">';
	...
	jQuery(document).on('click', '#migajaxmanager_shortcode_generator', function(){
		var $close = '<span class="migajaxmanager_close_shortcode">Close</span>';
		var $title = '<div class="migajaxmanager_shortcode_title">ajaxmanager Shortcode Options</div>';
		var $sendshortcode = '<span class="migajaxmanager_send_shortcode">Done</span>';
		...
		jQuery('body').append('<div class="migajaxmanager_shortcode_popup"><div class="migajaxmanager_shortcode_inner">'+$shortcode_options+'</div></div>');
		jQuery('.migajaxmanager_shortcode_inner').css({top: $windowHeight/2 - 250});
		jQuery('.migprog_colorpicker').wpColorPicker();
		
		// Remove Popup
		jQuery('.migajaxmanager_close_shortcode').on('click', function(){
			jQuery('.migajaxmanager_shortcode_popup').remove();
		})
		
		// Send Content to editor
		jQuery('.migajaxmanager_send_shortcode').on('click', function(){
			var $shortcode = '';
			var $column = jQuery('.migajaxmanager_shortcode_inner').find('#columnopt').val();
			var $style = jQuery('.migajaxmanager_shortcode_inner').find('#styleopt').val();
			var $design = jQuery('.migajaxmanager_shortcode_inner').find('#designopt').val();
			var $members = jQuery('.migajaxmanager_shortcode_inner').find('#membersopt').val();
			var $pagination = jQuery('.migajaxmanager_shortcode_inner').find('#paginationopt').val();
			var $orderby = jQuery('.migajaxmanager_shortcode_inner').find('#orderbyopt').val();
			var $order = jQuery('.migajaxmanager_shortcode_inner').find('#orderopt').val();
			var $spaces = jQuery('.migajaxmanager_shortcode_inner').find('#spacesopt').val();
			var $paginationback = jQuery('.migajaxmanager_shortcode_inner').find('#paginationback').val();
			var $paginationcolor = jQuery('.migajaxmanager_shortcode_inner').find('#paginationcolor').val();
			var $class = jQuery('.migajaxmanager_shortcode_inner').find('#classopt').val();
			
			$shortcode += '[ajaxmanager ';
